...
public class BPlusTree {
    ...
    // Constructors ////////////////////////////////////////////////////////////
    /**
     * Construct a new B+ tree with metadata `metadata` and lock context
     * `lockContext`.
     ...
     *
     * We additionally write a row to the _metadata.indices table with metadata
     * about
     * the B+ tree:
     *
     ...
     *
     * All pages allocated on the given partition are serializations of inner and
     * leaf nodes.
     */
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // the tree's ro ot if the old root splits.

        // Note that a check is done when an insertion happens at a LeafNode.

        Optional<Pair<DataBox, Long>> splitData = root.put(key, rid);
    ...
    /**
     * Note that the node that contains d entries (which is the old root) is going
     * to be
     * the child on the left of the new root where as the d + 1 entries node (the
     * new node) would be
     * the child on the right.
     * 
     * @param key      new key
     * @param recordId right_node_page_num
     */
    private void splitRoot(DataBox key, Long recordId) {
        List<DataBox> keys = new ArrayList<>();
        List<Long> children = new ArrayList<>();

        // Note that the root itself is modified when root.put method is used.
        // Moreover, it's splitted and the new root should now add the inner node's new
        // key.
        keys.add(key);
        // The left child of the first key of the new root should be the old root
        // (modified one actually) whereas the right child should point towards the new
        // node.

        children.add(root.getPage().getPageNum());
        children.add(recordId);
        InnerNode newRoot = new InnerNode(metadata, bufferManager, keys,
