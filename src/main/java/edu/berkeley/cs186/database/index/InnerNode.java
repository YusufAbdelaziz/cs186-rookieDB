...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> splitData = child.put(key, rid);
        if (splitData.isPresent()) {
            return insert(splitData.get().getFirst(), splitData.get().getSecond());
        } else {
    ...
    private Optional<Pair<DataBox, Long>> insert(DataBox key, Long child) {
        ...
        children.add(insertionIndex + 1, child);
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (!splitInfo.isPresent()) {
            // Bulk loading on child node is done and no need to split it.
            return splitInfo;
        } else {
            Pair<DataBox, Long> childSplitInfo = splitInfo.get();
            Optional<Pair<DataBox, Long>> currentNodeSplitInfo = insert(childSplitInfo.getFirst(),
                    childSplitInfo.getSecond());
            if (!currentNodeSplitInfo.isPresent()) {
                // If current node doesn't split, call bulkLoad recursively until it splits and
                // return to upper nodes.
                return bulkLoad(data, fillFactor);
            } else {
                return currentNodeSplitInfo;
            }
        }
    }
